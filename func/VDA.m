function [acc_s,acc_t] = VDA(Xs_l,Xs_u,Xt,Ys_l,Ys_u,Yt,options)
    ...
    for i = 1 : T
        ...
        for j=1:C
            Xj = Xs(:,find(Ys_pseudo==j));
            Hj = eye(size(Xj,2))-1/(size(Xj,2))*ones(size(Xj,2),size(Xj,2));
            Sws = Sws + Xj*Hj*Xj'; 
        end
        ...
        %% Calculation
        [A,~] = eigs(X*M*X'+lambda*eye(m)+gamma*(Sws),X*H*X',dim,'SM');
        Z = A'*X;
