function F = GeneralSSL4(A, T, is_normalize, delta_t,At,Ct)
...
G_pos4=2*delta_t*Ct'*Ct;
G_pos4_pos=(abs(G_pos4)+G_pos4)/2;
G_pos4_neg=(abs(G_pos4)-G_pos4)/2;
G_neg1=2*delta_t*At'*Ct;
G_neg1_pos=(abs(G_neg1)+G_neg1)/2;
G_neg1_neg=(abs(G_neg1)-G_neg1)/2;

for iter = 1:ITER
    obj(iter)=trace(F'*Ltt*F)+trace(Fs'*Lst*F)+trace(F'*Lts*Fs)+delta_t*trace((At-Ct*F')*(At-Ct*F')');
