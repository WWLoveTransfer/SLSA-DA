function [acc_s,acc_t] = BDA(Xs_l,Xs_u,Xt,Ys_l,Ys_u,Yt,options)
        ...
        if ~isempty(Yt_pseudo) && length(Yt_pseudo)==nt
            for c = reshape(unique(Ys_pseudo),1,C)
                ...
                if strcmp(mode,'W-BDA')
                    Ps = length(find(Ys_pseudo==c)) / length(Ys_pseudo);
                    Pt = length(find(Yt_pseudo == c)) / length(Yt_pseudo);
                    alpha = Pt / Ps;
                    mu = 1;
                else
                    alpha = 1;
                ...
                e(ns+find(Yt_pseudo==c)) = -alpha / length(find(Yt_pseudo==c));
                e(isinf(e)) = 0;
                N = N + e*e';
            end
        end
